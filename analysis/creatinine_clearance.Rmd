...
clop_ids <- fread("/home/rhea/account_wflow/vcfs/clop/clop_ids_CrCl.txt")
noac_ids <- fread("/home/rhea/account_wflow/vcfs/noac/noac_ids_CrCl.txt")

clop_noac_ids <- union_all(clop_ids, noac_ids)
fwrite(clop_noac_ids, "clop_noac_CrCl_ids.txt")
...
for chr in {1..22};
do vcftools --gzvcf /home/rhea/account_wflow/vcfs/blacks_imputed/CrCl/chr${chr}_maf01_r28.vcf.gz --keep /home/rhea/account_wflow/vcfs/clop_noac/clop_noac_CrCl_ids.txt --recode --recode-INFO-all --out clop_noac_chr${chr};
done

# Use vcf_to_dosage.py to output dosage txt files
for chr in {1..22};
do python /home/rhea/account_wflow/vcfs/vcf_to_dosage.py -i /home/rhea/account_wflow/vcfs/CrCl/clop_noac/ -c ${chr} -d clop_noac;
done
...
for model_pop in ${model_pops[@]}
  ...
  for drug in ${drugs[@]}
    ...
    --text_sample_ids /home/ashley/account/dosages/blacks/${drug}/samples_clop_noac.txt \
...
### Convert PrediXcan Output to GEMMA input - not done
...
for(model in model_list){
  for(drug in drug_list){
    pred_exp <- fread("/home/rhea/account_wflow/pwas_results/blacks/" %&% drug %&% "/predicted_levels/blacks_" %&% drug %&% "_PCAIR_PAV_filtered_" %&% model %&% "_WG_baseline_rho0.1_zpval0.05.txt", header = F) #path to PrediXcan-produced _predicted_expression.txt
    pred_exp$V1 <- NULL
    pred_exp$V2 <- NULL
